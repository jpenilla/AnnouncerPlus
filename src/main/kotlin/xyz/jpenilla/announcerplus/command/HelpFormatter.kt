...
class HelpFormatter(private val announcerPlus: AnnouncerPlus, manager: PaperCommandManager) : CommandHelpFormatter(manager) {
    ...
    override fun printDetailedHelpHeader(help: CommandHelp, issuer: CommandIssuer, entry: HelpEntry) {
        issuer.send(TextUtil.replacePlaceholders("<color:$color><strikethrough>-----</strikethrough><white>[</white> {commandprefix}{command} <white>Detailed Help ]</white><strikethrough>-----</strikethrough>", arrayToMap(getHeaderFooterFormatReplacements(help)), false))
    }

    override fun printSearchHeader(help: CommandHelp, issuer: CommandIssuer) {
        issuer.send(TextUtil.replacePlaceholders("<color:$color><strikethrough>-----</strikethrough><white>[</white> {commandprefix}{command} <italic>{search}</italic> <white>Search Results ]</white><strikethrough>-----</strikethrough>", arrayToMap(getHeaderFooterFormatReplacements(help)), false))
    }

    override fun printHelpHeader(help: CommandHelp, issuer: CommandIssuer) {
        issuer.send(TextUtil.replacePlaceholders("<color:$color><strikethrough>-----</strikethrough><white>[</white> {commandprefix}{command} <white>Help ]</white><strikethrough>-----</strikethrough>", arrayToMap(getHeaderFooterFormatReplacements(help)), false))
    }
    ...
    private fun getFooter(help: CommandHelp): String {
        ...
        builder.append("Page <color:$color>{page}</color:$color> of <color:$color>{totalpages}</color:$color> (<color:$color>{results} results<white>)</white> <strikethrough>----------</strikethrough>")
    ...
    override fun printDetailedHelpCommand(help: CommandHelp, issuer: CommandIssuer, entry: HelpEntry) {
        issuer.send(TextUtil.replacePlaceholders(" <white><color:$color>-</color:$color> <click:suggest_command:/{command} ><hover:show_text:'<italic>Click to suggest'>/</white><color:$color>{command}</color:$color> <gray>{parameters}</gray></hover></click> <color:$color>{separator}</color:$color> {description}", arrayToMap(getEntryFormatReplacements(help, entry)), false))
    }
